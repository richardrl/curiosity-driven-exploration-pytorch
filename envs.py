...
class PyGameEnvironment(Environment):
    def __init__(self, env_id, is_render, worker_idx, child_conn, history_size=4, h=84, w=84):
        super(PyGameEnvironment, self).__init__()
        assert h is not None, "Image height is none"
        assert w is not None, "Image width is none"
        self.env_id = env_id
        self.is_render = is_render
    ...
    def run(self):
        super(PyGameEnvironment, self).run()
        while True:
            action = self.child_conn.recv() # Line 133 in train.py sends action to us
            ...
            if done:
                ...
                print("[Episode {}({})] Step: {}  Reward: {}  Recent Reward: {}  ".format(
                    self.episode, self.worker_idx, self.steps, self.rall, np.mean(self.recent_rlist)
                    ))

                self.history = self.reset()

            self.child_conn.send(
                [self.history, rew, force_done, done, log_reward]
            )
